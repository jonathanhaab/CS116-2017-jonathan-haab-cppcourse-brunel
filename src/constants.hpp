...
constexpr double stopTime = 1210; //!< time at which we end the simulation
const double total_steps = std::abs(stopTime-startTime)/h; //!< total number of step of the simulation

constexpr double plotStartTime = 1000; //!< time at which we begin to catch data for the plot
constexpr double plotStopTime = 1200; //!< time at which we begin to catch data for the plot
...
constexpr double lambda(0.9); //! argument for poisson distribution
...
constexpr double J_i = -0.45; //!< the EPSP of an inhibitory neuron
